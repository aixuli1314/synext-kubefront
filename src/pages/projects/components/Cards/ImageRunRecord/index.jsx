...
import BuilderRunStore from 'stores/s2i/run'
...
export default class RunsCard extends React.Component {
  ...
  constructor(props) {
    ...
    this.store = new BuilderRunStore()
  }
  ...
  setAutoRefresh = () => {
    ...
    const hasRunning = data.some(
      run =>
        !['successful', 'failed'].includes(run.status) || isEmpty(run.status)
    )
    if (hasRunning) {
